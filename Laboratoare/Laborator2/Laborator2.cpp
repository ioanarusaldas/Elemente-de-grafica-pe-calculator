...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		vector<unsigned short> indices_T =
		{
		...
		vector<unsigned short> indices_S =
		{
		...
		Mesh* tet = CreateMesh("tet", vertices_T, indices_T);
		Mesh* square = CreateMesh("square", vertices_S, indices_S);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(0, 0, 1), glm::vec3(0.25f));

	RenderMesh(meshes["tet"], shaders["VertexColor"], glm::vec3(1, 1, 1));
