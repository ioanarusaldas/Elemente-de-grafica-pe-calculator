...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	//Bonus1
	if (moveright < 10) {
		moveright += deltaTimeSeconds;
	}
	else {
		moveright = 0;
	}
		moveup += deltaTimeSeconds;
		translateUp = abs(cos(moveup));
		r += deltaTimeSeconds;

	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(0, 0.5f, 1);
	modelMatrix *= Transform3D::Translate(moveright,0, 0);
	modelMatrix *= Transform3D::Translate(0, translateUp, 0);
	modelMatrix *= Transform3D::RotateOZ(-r);
